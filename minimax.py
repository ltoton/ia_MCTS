...
# Fonction Minimax
def minimax(plateau, profondeur, joueur_max, alpha, beta):
    ...
    if joueur_max == 1:
        ...
        for colonne in get_colonnes_valides(plateau):
            ...
            score = minimax(nouveau_plateau, profondeur - 1, 2, alpha, beta)
        ...
        for colonne in get_colonnes_valides(plateau):
            nouveau_plateau = plateau.copy()
            placer_jeton(nouveau_plateau, colonne, joueur_max)
            score = minimax(nouveau_plateau, profondeur - 1, 1, alpha, beta)
...
def obtenir_meilleur_coup(plateau, profondeur):
    ...
    for colonne in colonnes_valides:
        ...
        score = minimax(nouveau_plateau, profondeur - 1, 2, float("-inf"), float("inf"))
...
# Lancer le jeu avec l'IA utilisant Minimax
